...
class CmdStatus(CmdDefault):
    def method(self, update, context):
        ...
        reply += '\n' + self.vatb.fr.status_string
        if self.vatb.fr.running is True:
            reply += '\n' + self.vatb.fr.source_info_string
        context.bot.send_message(chat_id=chat_id, text=reply)

class CmdStart(CmdDefault):
    def method(self, update, context):
        chat_id = update.effective_chat.id
        if self.vatb.fr.running:
            reply = 'Face recognizer is already running.'
        else:
            reply = 'OK. Starting face recognizer.'
        if chat_id != self.vatb.alarm_receiver:
            reply += '\nVisitor alarm will be sent to you.'
            self.vatb.alarm_receiver = chat_id
        context.bot.send_message(chat_id=chat_id, text=reply)
        if not self.vatb.fr.running:
            self.vatb.fr.start_running()

class CmdStop(CmdDefault):
    def method(self, update, context):
        chat_id = update.effective_chat.id
        reply = self.vatb.fr.settings.__repr__()
        if self.vatb.fr.running:
            reply = 'OK. Stopping face recognizer.'
        else:
            reply = 'Face recognizer is not running now.'
        context.bot.send_message(chat_id=chat_id, text=reply)
        if self.vatb.fr.running:
            self.vatb.fr.stop_running()
...
class VisitorAlarmTelegramBot(face_recog.Observer):
    def __init__(self, face_recog, person_db, settings):
        self.fr = face_recog
        self.pdb = person_db
    ...
    def on_new_person(self, person):
        ...
        reply = person.name + " appeared"
        image = person.get_montage(1,1)
        is_success, buf = cv2.imencode(".jpg", image)
        bio = io.BytesIO(buf)
        bio.seek(0)
    ...
    def on_start(self, fr):
        chat_id = self.alarm_receiver
        reply = 'Face recognition is started.'
        reply += '\n' + fr.source_info_string
        self.core.send_message(chat_id=chat_id, text=reply)
        logging.info(reply)

    def on_stop(self, fr):
        chat_id = self.alarm_receiver
        reply = 'Face recognizer is stopped.'
        self.core.send_message(chat_id=chat_id, text=reply)
        logging.info(reply)
        self.alarm_receiver = None
    ...
    def str_settings(self):
        ...
        if self.settings.srcfile == 0:
            s += str(self.settings.srcfile) +' (webcam)'
...
if __name__ == '__main__':
    ...
    args = ap.parse_args()
    dir_name = "/Users/johnsmac/visitor_alarm_telegram_bot/result"
    pdb = person_db.PersonDB()
    pdb.load_db()
    fr = face_recog.FaceRecog(pdb,args)
    #fc = fr.Facerecognizer(pdb, args)
    vatb = VisitorAlarmTelegramBot(fr, pdb, args)
